from pyrogram import Client as Mbot, filters
...
async def _(Mbot,message):
    await message.reply_text(
        text=f"Youtube Support Disabled Due To Shaktiman Army !!", 
        parse_mode="markdown",
        quote=True  
    )
...
#                await message.reply(f"400: Sorry, Unable To Find It  try another or report it  to @THE_DS_OFFICIAL 🤖")
...
#             AForCopy = await message.reply_audio(fileLink,caption=f"[{id[3]}](https://youtu.be/{id[0]}) - {id[2]} Thank you for using Our Bot!",title=id[3].replace("_"," "),performer=id[2],thumb=thumnail,duration=id[4])
#             if DUMP_GROUP:
