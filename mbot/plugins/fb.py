    ...
    try:
       ...
       if get_api['success'] == "ok":
          if get_api.get('result').get('hd'):
             try:
                 dump_file = await message.reply_video(get_api['result']['hd'],caption="<b>Thank You For Using - @Public_Media_Downloader_Bot</b>")
             except KeyError:
                 pass 
             except Exception:
                 try:
                     ...
                     try:
                        ...
                        await message.reply_video(down_file,caption="<b>Thank You For Using - @Public_Media_Downloader_Bot</b>")
                        await sndmsg.delete()
                        os.remove(down_file)
                     except:
             ...
             if get_api.get('result').get('sd'):
               try:
                   dump_file = await message.reply_video(get_api['result']['sd'],caption="<b>Thank You For Using - @Public_Media_Downloader_Bot</b>")
               except KeyError:
                   pass
               except Exception:
                   try:
